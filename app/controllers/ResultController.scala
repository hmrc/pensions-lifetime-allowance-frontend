...
class ResultController @Inject() (
  ...
  val processFPApplication = Action.async { implicit request =>
    authFunction.genericAuthWithNino("FP2016") { nino =>
      ...
      plaConnector.applyFP16(nino).flatMap(response => routeViaMCNeededCheck(response, nino))
    }

  }

  val processIPApplication = Action.async { implicit request =>
  ...
  private[controllers] def routeViaMCNeededCheck(
      response: HttpResponse,
      nino: String
  )(implicit request: Request[AnyContent], protectionType: ApplicationType.Value): Future[Result] =
    response.status match {
      case 423 => Future.successful(Locked(manualCorrespondenceNeeded()))
      case _   => saveAndRedirectToDisplay(response, nino)
    }

  private[controllers] def saveAndRedirectToDisplay(
      response: HttpResponse,
      nino: String
  )(implicit request: Request[AnyContent], protectionType: ApplicationType.Value): Future[Result] =
    responseConstructors
      .createApplyResponseModelFromJson(response.json)
      .map { model =>
        if (model.protection.notificationId.isEmpty) {
          ...
          sessionCacheService
            .saveFormData[ApplyResponseModel](common.Strings.nameString("applyResponseModel"), model)
            .map { cacheMap =>
      ...
      .getOrElse {
        logger.warn(
          s"Unable to create ApplyResponseModel from application response for ${protectionType.toString} for user nino: $nino"
        )
        Future.successful(
    ...
    implicit request =>
      ...
      authFunction.genericAuthWithNino("existingProtections") { nino =>
        ...
        val errorResponse =
          InternalServerError(technicalError(protectionType.toString)).withHeaders(CACHE_CONTROL -> "no-cache")
        sessionCacheService
          .fetchAndGetFormData[ApplyResponseModel](common.Strings.nameString("applyResponseModel"))
          .map {
            case Some(model) =>
  ...
  def applicationOutcome(
      notificationId: Int
  )(implicit protectionType: ApplicationType.Value): ApplicationOutcome.Value = {
  ...
  private def successfulOutcomeType(notificationId: Int): ApplicationOutcome.Value =
    if (Constants.inactiveSuccessCodes.contains(notificationId)) ApplicationOutcome.SuccessfulInactive
    else ApplicationOutcome.Successful
