...
class WithdrawProtectionDateInputConfirmationController @Inject() (
  ...
  private[controllers] def fetchWithdrawDateForm(
  ...
  private def buildInvalidForm(
      errorForm: Form[WithdrawDateFormModel]
  )(implicit messages: Messages): Form[WithdrawDateFormModel] =
    if (errorForm.errors.size > 1) {
      ...
      val finalErrors: Seq[FormError] = formFieldsWithErrors ++ Seq(
        FormError("withdrawDate.day", Messages("pla.withdraw.date-input-form.date-invalid"))
      )
  ...
  def getWithdrawDate(form: Form[WithdrawDateFormModel]): String =
    Dates.apiDateFormat(
      form.get.withdrawDate.getDayOfMonth,
      form.get.withdrawDate.getMonthValue,
      form.get.withdrawDate.getYear
    )
