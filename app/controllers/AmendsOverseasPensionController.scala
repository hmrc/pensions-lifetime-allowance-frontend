      ...
      authFunction.genericAuthWithNino("existingProtections") { nino =>
        fetchAmendProtectionModel(protectionType, status)
          .map {
                    ...
                    amendOverseasPensions(
                      amendOverseasPensionsForm(IndividualProtection2016.toString).fill(
                        AmendOverseasPensionsModel(
                          yesNoValue,
                          Some(
                            Display.currencyInputDisplayFormat(data.updatedProtection.nonUKRights.getOrElse[Double](0))
                          )
                        )
                      ),
            ...
            case _ =>
              logger.warn(
                s"Could not retrieve amend protection model for user with nino $nino when loading the amend overseasPension page"
              )
              ...
              fetchAmendProtectionModel(protectionType, status)
                .flatMap {
                  ...
                  case _ =>
                    ...
                    Future.successful(
                      InternalServerError(technicalError(ApplicationType.IP2016.toString))
                        .withHeaders(CACHE_CONTROL -> "no-cache")
                    )
                }
          )
