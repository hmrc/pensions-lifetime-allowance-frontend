...
class AmendCurrentPensionFormSpec extends FakeApplication with CommonErrorMessages with MockitoSugar {
  ...
  private val messageKey = "currentPensions"
  ...
  "Amend current pensions form" should {
    ...
    "create a valid form" when {
      ...
      "supplied with a valid map with the maximum amount" in {
        val map    = validMap.updated("amendedUKPensionAmt", Constants.maximumCurrencyAmount.toString)
        val result = amendCurrentPensionForm(IndividualProtection2016).bind(map)

        result.value shouldBe Some(AmendCurrentPensionModel(Some(Constants.maximumCurrencyAmount)))
      }
    }
    ...
    "create an invalid form" when {
      ...
      "supplied with an amount above the maximum" in {
        val map    = validMap.updated("amendedUKPensionAmt", (Constants.maximumCurrencyAmount + 1).toString)
        val result = amendCurrentPensionForm(IndividualProtection2016).bind(map)
