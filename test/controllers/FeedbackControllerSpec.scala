...
class FeedbackControllerSpec extends UnitSpec with MockitoSugar with WithFakeApplication {
  ...
  "GET /feedback" should {
    ...
    "capture the referrer in the session on the feedback load" in {
      val result = target.show(fakeRequest.withHeaders("Referer" -> "Whatever"))
      status(result) shouldBe Status.OK
    }
  }

  "POST /feedback" should {
    val fakePostRequest = FakeRequest("POST", "/protect-your-lifetime-allowance/feedback").withFormUrlEncodedBody("test" -> "test")
  ...
  "GET /protect-your-lifetime-allowance/thankyou" should {
    "should return the thankyou page" in {
