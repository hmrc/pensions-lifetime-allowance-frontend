...
class SessionRepositoryISpec
  ...
  "getFromSession" must {
    "return None when no data for this session exists" in {
      repository.putInSession(testAnswerKey, testAnswer)(PensionsTakenModel.format, otherUserRequest, global).futureValue
  ...
  "clearSession" must {
    "successfully remove session of a user" in {
      ...
      repository.getFromSession[PensionsTakenModel](testAnswerKey)(PensionsTakenModel.format, userRequest).futureValue shouldBe None
      repository.getFromSession[PensionsTakenModel](testAnswerKey)(PensionsTakenModel.format, otherUserRequest).futureValue shouldBe Some(testAnswer)
    }
  }
}
